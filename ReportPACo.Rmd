...
setwd("C:/Users/anmis/Desktop/trees")
```
...
hosts<-paste0(rep(" %s", length(TreeH$tip.label)), collapse=",")
symb<-paste0(rep(" %s", length(TreeS$tip.label)), collapse=",")

cap1<-paste0("Tanglegram of the associations between the hosts:", hosts, " and the symbionts:", symb, collapse = "")
hosts<-TreeH$tip.label
symb<-TreeS$tip.label
```
...
phylogenies. The number of species of the host phylogeny is: `r length(TreeH$tip.label)`,
the number of species of the symbiont phylogeny is `r length(TreeS$tip.label)`.
The number of associations between the two phylogenies is `r sum(HS)`.
...
```{r, computing the procrustes sum square residuals}
N= params$reps
n=params$n
...
```{r, fig.cap="Distribution of the SSR of PACo analyses"}
dens<-density(PACO)
quan<-quantile(PACO, probs = 0.01)

plot(dens, main = "Density plot of the distribution of the SSR of PACo",
...
treeS= mTreeS[[i]], symmetric=sim, proc.warns = warn)
...
cap4<-sprintf("Average residual frequency distributions (observed – expected frequencies) of host-symbiont applying Random TaPas with PACo , N = %s, n = %.0f and p = %.2f. Vertical lines represent 95 percent confidence intervals of the residual frequencies
computed empirically with %.0f randomly chosen pairs of trees used to build the consensus trees", N, n, perc, length(mTreeH))
```

# Residual frequencies per association plot

```{r, plotting OE frequencies, fig.cap=cap4}
op <- par(mgp = c(2.2, 0.3, 0), mar=c(3.5,3.2,3,1), tck=0.01,
oma = c(5.4, 0, 0, 0), xpd=NA)
link.fq <-barplot(PACO.AV, xaxt='n',
horiz=FALSE, cex.names = 0.6, las=2, cex.axis=0.8,
ylab="Observed - Expected frequency",col="lightblue", main = "Average Residual Frequencies per association with PACo")
...
tick = FALSE, line= 1, cex.axis=0.6)
par(op)

cap5<-sprintf("Normalized Gini coefficient of the residual frequency distributions applying Random TaPas with PACo to the consensus trees (asterisk) and to %.0f randomly chosen trees used to build the consensus trees", length(mTreeH))
```
